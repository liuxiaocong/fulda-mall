...
class ShopSetMobilePageView extends React.Component {
    ...
    onTapSave() {
        if ( !this.state.mobile || this.state.mobile.length <= 0 ) {
            this.setState( { error: I18n.t( Keys.plz_set_phone ) } );
            return;
        }

        this.setState( { isRequesting: true, error: '' } );
        this.props.onTapSave(
            this.state.mobile, ( err, resBody ) => {
                this.setState( { isRequesting: false } );
                if ( err ) {
                    Toast.show( I18n.t( Keys.update_fail ) )
                } else {
...
export default ShopSetMobilePageView;
