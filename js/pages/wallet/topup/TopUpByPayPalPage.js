...
class TopUpByPayPalPage extends React.Component {
    ...
    onShouldStartLoadWithRequest( navState ) {
        ...
        if ( navState.url.indexOf( 'paypal/success' ) > 0 ) {
            this.onTapTopUpSuccess();
            return false;
        } else if ( navState.url.indexOf( 'paypal/failed' ) > 0 ) {
            if ( this.state.callback instanceof Function ) {
                this.state.callback( false );
            }
            this.props.navigation.goBack();
            return false;
        } else if ( navState.url.indexOf( 'paypal/cancelled' ) > 0 ) {
            this.props.navigation.goBack();
            return false;
        }
...
export default  connect( select )( TopUpByPayPalPage );
